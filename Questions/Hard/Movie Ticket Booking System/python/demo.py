...
class MovieTicketBookingDemo:
    ...
    def run():
        ...
        # Add shows
        show1 = Show("S1", movie1, theater1, datetime.now(),
                     datetime.now() + timedelta(minutes=movie1.duration_in_min), create_seats(10, 10))
        show2 = Show("S2", movie2, theater2, datetime.now(),
                     datetime.now() + timedelta(minutes=movie2.duration_in_min), create_seats(8, 8))
